Public Class EmployeesManager
    ...
    Public Shared Function store(employee As Employee) As Boolean
        Try
            command = New MySql.Data.MySqlClient.MySqlCommand("INSERT INTO Employees(last_name, first_name, phone_number, email, gender, function_field) VALUES(@last_name, @first_name, @phone_number, @email, @gender, @function_field);", Manager.connection)
    ...
    Public Shared Function update(employee As Employee, id As Integer) As Boolean
        Try
            command = New MySql.Data.MySqlClient.MySqlCommand("UPDATE Employees SET last_name = @last_name, first_name = @first_name, phone_number = @phone_number, email = @email, gender = @gender, function_field = @function_field WHERE id = @id;", Manager.connection)
            command.Parameters.AddWithValue("@last_name", employee.LastName)
            command.Parameters.AddWithValue("@first_name", employee.FirstName)
            command.Parameters.AddWithValue("@phone_number", employee.PhoneNumber)
            command.Parameters.AddWithValue("@email", employee.Email)
